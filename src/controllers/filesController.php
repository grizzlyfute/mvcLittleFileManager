...
class FilesController extends BaseController
{
  ...
  private function readDirEntries(string $syspath): array
  {
    ...
    {
      throw new BadRequestException('Can not open dir "' . $syspath . '"');
    }
  ...
  private function rchmod(string $syspath, int $filemode, int $dirmode): bool
  {
    ...
    elseif (is_link($syspath))
    {
      return true;
    }
    elseif (is_file($syspath))
    {
      return chmod($syspath, $filemode);
    }
    return false;
  }
  ...
  public function listAction()
  {
    ...
    {
      if ($x->getIsDir() == $y->getIsDir()) return strcasecmp($x->getName(), $y->getName());
  ...
  public function thumbnailAction()
  {
    ...
    switch ($type)
    {
      ...
      case IMAGETYPE_BMP: // 6
        $extractorFn = 'imagecreatefrombmp';
        $writerFn = 'imagebmp';
        break;
  ...
  public function uploadAction()
  {
      ...
      (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'))
    {
      ...
      if ($success)
      {
        if (filesize($temp_file) > $maxSize)
        {
          $err = 'File is too big. Max allowed size is ' . ($maxSize / (1024*1024)) . ' Mb';
          $success = false;
        }
      }
    ...
    else if (!empty($_FILES))
    {
      ...
      {
        if (filesize($tmp_name) > $maxSize)
        {
          throw new BadRequestException ('File is too big. Max allowed size is ' . ($maxSize / (1024*1024)) . ' Mb');
        }
      ...
      {
        throw new BadRequestException('Error while uploading "' . $_FILES['file']['name'] . '" ' . $_FILES['error']);
      }
      $fileinfo = new stdClass();
