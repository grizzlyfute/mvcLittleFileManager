...
function fileActionPreparePre(filepath, action)
{
  ...
  filepath = extractFromHtml(filepath);
        ...
        <div class="modal-body">
          ...
          <div class="container">
            ...
            <div id="fileActionDstDiv" class="row" style="display:none;">
              ...
              <div class="col-sm-1">
                <button type="button" id="btnSelectDstDir" class="rounded" onClick="selectDstDir(removeUserRoot('<?php echo utils_escapeSingleQuote($path)?>'), true);"
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="tab-content">
            <p>
              <?php echo $tr->trans('file.destination') ?>: <?php echo utils_pathToHtml($path) ?>
            </p>
            ...
            <div class="tab-pane fade upload-url-wrapper" id="urlUploader" aria-labelledby="urlUploadTab">
              <form id="js-form-url-upload" class="form-inline" onsubmit="return upload_from_url(this);" method="POST" action="?action=upload">
                <input type="hidden" name="p" value="<?php echo utils_pathToHtml($path) ?>"/>
...
{
  timeout: 900000,
  maxFilesize: <?php echo $CONFIG['maxuploadsizemb']; ?>,
  parallelUploads: 2,
  ...
  init: function ()
  {
    ...
    }).on("error", function(file, response)
    {
      ...
      if (response != null) alert (response.replace(/<[^>]*>/g, ''));
      else alert ("error");
    });
  }
}
          ...
          <div class="dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownFileAction">
              <li><a title="<?php $tr->trans('file.copy')?>"
                ...
                class="dropdown-item"
                onclick="fileActionPreparePre('<?php echo utils_escapeSingleQuote($item->getFullPath()) ?>', 'copy')">
                <i class="fas fa-copy"></i>&nbsp;<?php $tr->trans('file.copy') ?>
              </a></li>
              <li><a title="<?php $tr->trans('file.moverename')?>"
                href="#" data-bs-toggle="modal" data-bs-target="#fileActionModal"
                class="dropdown-item"
                onclick="fileActionPreparePre('<?php echo utils_escapeSingleQuote($item->getFullPath()) ?>', 'moverename')">
                <i class="fas fas fa-people-carry"></i>&nbsp;<?php $tr->trans('file.moverename') ?>
              </a></li>
              <li><a title="<?php $tr->trans('file.delete')?>"
                href="#" data-bs-toggle="modal" data-bs-target="#fileActionModal"
                class="dropdown-item"
                onclick="fileActionPreparePre('<?php echo utils_escapeSingleQuote($item->getFullPath()) ?>', 'delete')">
              ...
              <li><a title="<?php $tr->trans('file.extract')?>"
                href="#" data-bs-toggle="modal" data-bs-target="#fileActionModal"
                class="dropdown-item"
                onclick="fileActionPreparePre('<?php echo utils_escapeSingleQuote($item->getFullPath()) ?>', 'extract')">
